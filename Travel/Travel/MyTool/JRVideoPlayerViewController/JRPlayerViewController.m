...
@property (nonatomic, strong) UIAlertView *alertView;// 提醒视图
...
- (void)playerItemDidPlayToEnd:(NSNotification *)aNotification {
    ...
    self.playing = NO;
    self.alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"您要重新播放吗?" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:@"取消", nil];
    [self.alertView show];
}
    ...
    if (context == playerItemStatusContext) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([[change objectForKey:@"new"] intValue] == 1) {
                ...
                //_timer = [NSTimer scheduledTimerWithTimeInterval:3 target:self selector:@selector(bofang) userInfo:@"123" repeats:YES];
            }
        });
